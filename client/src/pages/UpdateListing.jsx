...
export default function CreateListing() {
    ...
    const handleImageSubmit = (e) => {
        if (files.length > 0 && files.length + formData.imageUrls.length < 7){
            ...
            .catch((err)=> {setImageUploadError('image upload failed');
            setUploading(false);
        });
        } else{
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto'>
      <h1 className='text-center text-3xl font-semibold my-7'>Update Listing</h1>
      <form onSubmit = {handleSubmit} className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex flex-col flex-1 gap-4'>
            ...
            <div className='flex gap-4 '>
                <input onChange={(e) => setFiles(e.target.files) } className='p-3 border border-gray-300 ro' type = "file" id = 'images' accept = 'image/*' multiple />
            ...
            {error && <p className="text-red-700text-sm">{error}</p>}
        </div>
      </form>
    </main>
  )
